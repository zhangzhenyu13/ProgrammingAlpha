...
def build_encoderV2(opt, embeddings):
    '''
    extended version of build_encoder
    '''
    pretrained_encoder = opt.pretrained_encoder
    
    if pretrained_encoder in supported_encoder_builders.keys():
        logger.info("building with extended encoder version:{}".format(pretrained_encoder))
        return supported_encoder_builders[pretrained_encoder]()
    else:
        logger.info("building with system built-in encoder")
        return build_encoder(opt, embeddings)
...
def build_decoder(opt, embeddings):
    ...
    logger.info("decoder type is : {} ({},{})".format(dec_type, opt.decoder_type, opt.input_feed))
...
def build_base_model(model_opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    # Build encoder.
    encoder = build_encoderV2(model_opt, src_emb)
    # Build decoder.
    ...
    print("word padding idx=", tgt_emb.word_padding_idx)
