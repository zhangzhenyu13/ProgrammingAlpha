...
def fetchQuestionData():
    questionsData= docDB_local.loadCollection(docDB_local.questions)
...
def fetchAnswerData():
    answersData= docDB_local.loadCollection(docDB_local.answers)
...
def generateContextAnswerCorpusParallel(questionsDataGlobal,answersDataGlobal,indexDataGlobal):
    ...
    with open(programmingalpha.DataPath+"Corpus/"+args.db.lower()+"-posts.json","w") as f:
        for batch_qids in tqdm.tqdm(batches,desc="processing documents multi-process"):
...
def generateContextAnswerCorpus(questionsDataGlobal,answersDataGlobal,indexDataGlobal):
    ...
    with open(programmingalpha.DataPath+"Corpus/"+args.db.lower()+"-posts.json","w") as f:
        for q_id in tqdm.tqdm(question_ids,desc="processing documents single"):
...
def main():
    ...
    indexerDataGlobal=fetchIndexData()

    if args.workers>1:
        generateContextAnswerCorpusParallel(questionsDataGlobal,answersDataGlobal,indexerDataGlobal)
    else:
        generateContextAnswerCorpus(questionsDataGlobal,answersDataGlobal,indexerDataGlobal)
